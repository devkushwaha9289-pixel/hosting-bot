...
def scan_zip_security(zip_path):
    ...
    r'\bcompile\b',
    r'\b__import__\b',
    r'\bshutil\.(rmtree|copytree|move)\b',
    r'\bos\.environ\b',
    r'\bsocket\b',
    r'\brequests\b',
    r'\bparamiko\b',
...
def create_admin_panel():
    ...
    markup.row(
        types.InlineKeyboardButton('💾 Backup All Data', callback_data='backup_data'),
        types.InlineKeyboardButton('♻️ Restore Data', callback_data='restore_data')
    )
    markup.row(types.InlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main'))
    return markup
...
def _logic_backup(message):
    user_id = message.from_user.id
    if user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    
    bot.reply_to(message, "⏳ Creating full backup... This may take a while.")
    logger.info(f"Backup started by admin {user_id}")
    ...
    try:
        ...
        with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            if os.path.exists(DATABASE_PATH):
                zf.write(DATABASE_PATH, 'inf/bot_data.db')
                file_count += 1
                total_size += os.path.getsize(DATABASE_PATH)
        ...
        logger.info(f"Backup created: {backup_path} ({backup_size_mb:.2f} MB, {file_count} files)")
        ...
        with open(backup_path, 'rb') as f:
            caption = (
                ...
                f"⚠️ Keep this file safe! Use `/restore` to restore."
            )
            bot.send_document(message.chat.id, f, caption=caption, parse_mode='Markdown')
        
        try:
            backups = sorted(
                [os.path.join(BACKUP_DIR, f) for f in os.listdir(BACKUP_DIR) if f.startswith('full_backup_') and f.endswith('.zip')],
        ...
        logger.info(f"Backup sent successfully to admin {user_id}")
        
    except Exception as e:
        logger.error(f"Backup error: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Backup failed: {str(e)}")

def _logic_restore(message):
    user_id = message.from_user.id
    if user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    ...
    warning_text = (
        ...
        "📤 Send the backup `.zip` file to restore.\n"
        "   (Must be created by `/backup` command)\n\n"
        "🛑 All running scripts will be stopped.\n"
        "💾 Current DB will be saved as `.pre_restore` as safety.\n\n"
        "Send `/cancel` to abort."
    )
    msg = bot.reply_to(message, warning_text, parse_mode='Markdown')
...
def process_restore_file(message):
    ...
    if not message.document:
        bot.reply_to(message, "❌ Please send a valid ZIP backup file. Restore aborted.")
        return
    
    doc = message.document
    file_name = doc.file_name or "backup.zip"
    
    if not file_name.lower().endswith('.zip'):
        bot.reply_to(message, "❌ File must be a `.zip` backup file. Restore aborted.")
        return
    
    if doc.file_size > 100 * 1024 * 1024:
        bot.reply_to(message, "❌ Backup too large (max 100MB).")
        return
    
    temp_dir = None
    try:
        status_msg = bot.reply_to(message, "⏳ Downloading backup file...")
        ...
        bot.edit_message_text("🔍 Validating backup...", message.chat.id, status_msg.message_id)
        ...
        with zipfile.ZipFile(zip_path, 'r') as zf:
            ...
            db_entry = None
            for n in names:
                if n.endswith('bot_data.db'):
                    db_entry = n
                    break
            
            if not db_entry:
                bot.edit_message_text("❌ Invalid backup: No database (`bot_data.db`) found!", 
                                      message.chat.id, status_msg.message_id)
                return
            
            zf.extractall(extract_dir)
        
        bot.edit_message_text("🛑 Stopping all running scripts...", message.chat.id, status_msg.message_id)
        ...
        for key in list(bot_scripts.keys()):
            try:
                ...
                logger.error(f"Error stopping {key} during restore: {e}")
        
        logger.info(f"Stopped {stopped} scripts before restore")
        
        bot.edit_message_text("💾 Backing up current database...", message.chat.id, status_msg.message_id)
        
        if os.path.exists(DATABASE_PATH):
            try:
                safety_path = DATABASE_PATH + f".pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                shutil.copy2(DATABASE_PATH, safety_path)
                logger.info(f"Safety backup created: {safety_path}")
            except Exception as e:
                logger.error(f"Failed to create safety DB backup: {e}")
        
        bot.edit_message_text("📥 Restoring database...", message.chat.id, status_msg.message_id)
        
        new_db_path = None
        for root, dirs, files in os.walk(extract_dir):
            for file in files:
                if file == 'bot_data.db':
                    new_db_path = os.path.join(root, file)
                    break
            if new_db_path:
                break
        ...
        if new_db_path and os.path.exists(new_db_path):
            ...
            shutil.copy2(new_db_path, DATABASE_PATH)
            logger.info(f"Database restored from {new_db_path}")
        
        bot.edit_message_text("📁 Restoring user files...", message.chat.id, status_msg.message_id)
        
        restored_files = 0
        upload_extract = None
        
        for root, dirs, files in os.walk(extract_dir):
            if os.path.basename(root) == 'upload_bots':
                upload_extract = root
                break
        
        if upload_extract and os.path.exists(upload_extract):
            if os.path.exists(UPLOAD_BOTS_DIR):
                try:
                    shutil.rmtree(UPLOAD_BOTS_DIR)
                except Exception as e:
                    logger.warning(f"Could not fully clear upload_bots: {e}")
            os.makedirs(UPLOAD_BOTS_DIR, exist_ok=True)
            
            for item in os.listdir(upload_extract):
                src = os.path.join(upload_extract, item)
            ...
            logger.info(f"Restored {restored_files} items to upload_bots")
        
        bot.edit_message_text("🔄 Reloading in-memory data...", message.chat.id, status_msg.message_id)
        ...
        try:
            ...
            if ADMIN_ID != OWNER_ID:
                c.execute('INSERT OR IGNORE INTO admins (user_id, added_by, added_date) VALUES (?, ?, ?)', 
                          (ADMIN_ID, OWNER_ID, now_iso))
            conn.commit()
            conn.close()
        except Exception as e:
            logger.error(f"Error ensuring owner/admin in restored DB: {e}")
        ...
        admin_ids.clear()
        admin_ids.update({OWNER_ID, ADMIN_ID})
        ...
        success_text = (
            ...
            f"📁 User File Folders: `{restored_files}`\n"
            f"📢 Mandatory Channels: `{len(mandatory_channels)}`\n"
            f"⚙️ Custom Limits: `{len(user_limits)}`\n\n"
            f"🛑 Scripts Stopped: `{stopped}`\n"
            f"ℹ️ Users need to re-start their scripts manually."
        )
        ...
        logger.warning(f"✅ Restore completed successfully by admin {user_id}")
        
    except zipfile.BadZipFile:
        bot.reply_to(message, "❌ Invalid/corrupted ZIP file.")
        logger.error("Restore failed: Bad zip file")
    except Exception as e:
...
def _logic_help(message):
    help_text = """
🤖 **VILLAGEE X Hosting Bot Help Guide**
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🆘 Help": _logic_help,
    "💾 Backup": _logic_backup,
    "♻️ Restore": _logic_restore
}
...
def command_backup(message): _logic_backup(message)
@bot.message_handler(commands=['restore'])
def command_restore(message): _logic_restore(message)
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'backup_data': admin_required_callback(call, backup_data_callback)
        elif data == 'restore_data': admin_required_callback(call, restore_data_callback)
...
def backup_data_callback(call):
    """Handle backup button click"""
    bot.answer_callback_query(call.id, "💾 Creating backup...")
    _logic_backup(call.message)

def restore_data_callback(call):
    """Handle restore button click"""
    bot.answer_callback_query(call.id, "♻️ Starting restore process...")
    _logic_restore(call.message)
